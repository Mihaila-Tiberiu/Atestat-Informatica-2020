...
$mysqli = new mysqli('localhost','root','','oltenita');
	if(!isset($_SESSION['ins3']))
	{ 
		header('Location: index.php');
	?></head><?php
	}
	else{
$sql="SELECT * FROM recenziimuzee WHERE idInstitutie='".$_SESSION['ins3']."';" ;
$rs=$mysqli->query($sql);

$sql2="SELECT * FROM muzee WHERE idMuzeu='".$_SESSION['ins3']."';" ;
...
<header>
	<a href="muzee.php"><input type="button" value="Inapoi la lista de muzee!"></a> - Recenzii <?php echo $institutie; ?> <!-- -->
	<?php 
	...
	if(isset($_POST['elog']))
	{
		...
		header('Location: getinsmuzee.php');
	}
	?>
	</div>
...
<body>
	...
	<table class="center" style="width:90%">
		...
		<tbody>
			...
			{	
				$sql6="DELETE FROM `recenziimuzee` WHERE `idRecenzie`=".$_POST['eadmin'].";";
				echo $sql6;
				$mysqli->query($sql6);
				$_POST['eadmin']=-1;
				header('Location: getinsmuzee.php');
	...
	<?php 
		...
		{
		$ok=1;
			$rs4=$mysqli->query("SELECT * FROM recenziimuzee WHERE idInstitutie='".$_SESSION['ins3']."';");
		...
		if($ok==1){
		$sql4="INSERT INTO `recenziimuzee`(`username`, `idInstitutie`, `comentariu`, `stele`) VALUES ('".$_SESSION['username']."','".$_SESSION['ins3']."','".trim($_POST['comentariu'])."','".$_POST['stele']."');";
		$mysqli->query($sql4);
		header("Location: getinsmuzee.php");
		}
		else{
