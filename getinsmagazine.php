...
$mysqli = new mysqli('localhost','root','','oltenita');
	if(!isset($_SESSION['ins2']))
	{ 
		header('Location: index.php');
	?></head><?php
	}
	else{
$sql="SELECT * FROM recenziimagazine WHERE idInstitutie='".$_SESSION['ins2']."';" ;
$rs=$mysqli->query($sql);

$sql2="SELECT * FROM magazine WHERE idMagazin='".$_SESSION['ins2']."';" ;
...
<header>
	<a href="magazine.php"><input type="button" value="Inapoi la lista de magazine!"></a> - Recenzii <?php echo $institutie; ?> <!-- -->
	<?php 
	...
	if(isset($_POST['elog']))
	{
		...
		header('Location: getinsmagazine.php');
	}
	?>
	</div>
...
<body>
	...
	<table class="center" style="width:90%">
		...
		<tbody>
			...
			{	
				$sql6="DELETE FROM `recenziimagazine` WHERE `idRecenzie`=".$_POST['eadmin'].";";
				echo $sql6;
				$mysqli->query($sql6);
				$_POST['eadmin']=-1;
				header('Location: getinsmagazine.php');
	...
	<?php 
		...
		{
		$ok=1;
			$rs4=$mysqli->query("SELECT * FROM recenziimagazine WHERE idInstitutie='".$_SESSION['ins2']."';");
		...
		if($ok==1){
		$sql4="INSERT INTO `recenziimagazine`(`username`, `idInstitutie`, `comentariu`, `stele`) VALUES ('".$_SESSION['username']."','".$_SESSION['ins2']."','".trim($_POST['comentariu'])."','".$_POST['stele']."');";
		$mysqli->query($sql4);
		header("Location: getinsmagazine.php");
		}
		else{
