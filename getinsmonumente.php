...
$mysqli = new mysqli('localhost','root','','oltenita');
	if(!isset($_SESSION['ins5']))
	{ 
		header('Location: index.php');
	?></head><?php
	}
	else{
$sql="SELECT * FROM recenziimonumente WHERE idInstitutie='".$_SESSION['ins5']."';" ;
$rs=$mysqli->query($sql);

$sql2="SELECT * FROM monumente WHERE idMonument='".$_SESSION['ins5']."';" ;
...
<header>
	<a href="monumente.php"><input type="button" value="Inapoi la lista de monumente!"></a> - Recenzii <?php echo $institutie; ?> <!-- -->
	<?php 
	...
	if(isset($_POST['elog']))
	{
		...
		header('Location: getinsmonumente.php');
	}
	?>
	</div>
...
<body>
	...
	<table class="center" style="width:90%">
		...
		<tbody>
			...
			{	
				$sql6="DELETE FROM `recenziimonumente` WHERE `idRecenzie`=".$_POST['eadmin'].";";
				echo $sql6;
				$mysqli->query($sql6);
				$_POST['eadmin']=-1;
				header('Location: getinsmonumente.php');
	...
	<?php 
		...
		{
		$ok=1;
			$rs4=$mysqli->query("SELECT * FROM recenziimonumente WHERE idInstitutie='".$_SESSION['ins5']."';");
		...
		if($ok==1){
		$sql4="INSERT INTO `recenziimonumente`(`username`, `idInstitutie`, `comentariu`, `stele`) VALUES ('".$_SESSION['username']."','".$_SESSION['ins5']."','".trim($_POST['comentariu'])."','".$_POST['stele']."');";
		$mysqli->query($sql4);
		header("Location: getinsmonumente.php");
		}
		else{
