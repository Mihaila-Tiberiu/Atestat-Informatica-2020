...
$sql="SELECT * FROM recenziihoteluri WHERE idInstitutie='".$_SESSION['ins']."';" ;
$rs=$mysqli->query($sql);

$sql2="SELECT * FROM hoteluri WHERE idHotel='".$_SESSION['ins']."';" ;
...
<header>
	<a href="hoteluri.php"><input type="button" value="Inapoi la lista de hoteluri!"></a> - Recenzii <?php echo $institutie; ?> <!-- -->
	<?php 
	...
	if(isset($_POST['elog']))
	{
		...
		header('Location: getinshoteluri.php');
	}
	?>
	</div>
...
<body>
	...
	<table class="center" style="width:90%">
		...
		<tbody>
			...
			{	
				$sql6="DELETE FROM `recenziihoteluri` WHERE `idRecenzie`=".$_POST['eadmin'].";";
				echo $sql6;
				$mysqli->query($sql6);
				$_POST['eadmin']=-1;
				header('Location: getinshoteluri.php');
	...
	<?php 
		...
		{	
			$ok=1;
			$rs4=$mysqli->query($sql="SELECT * FROM recenziihoteluri WHERE idInstitutie='".$_SESSION['ins']."';");
		...
		if($ok==1){
		$sql4="INSERT INTO `recenziihoteluri`(`username`, `idInstitutie`, `comentariu`, `stele`) VALUES ('".$_SESSION['username']."','".$_SESSION['ins']."','".trim($_POST['comentariu'])."','".$_POST['stele']."');";
		$mysqli->query($sql4);
		header("Location: getinshoteluri.php");
		}
		else{
