...
<header>
	<a href="index.php"><input type="button" value="Inapoi Acasa"></a> - Muzee 
	<?php 
		...
		<tbody>
			...
			?>
				<tr>
					<td>
						<form action="" method="post">
							<input type="hidden" id="ins" name="ins" value='<?php echo $row['idMuzeu']; ?>'>
							<input type="submit" value="Vezi Recenziile!">
						</form>
						<?php
						if(isset($_POST['ins']))
						{
							$_SESSION['ins3']=$_POST['ins'];
					...
					<td style="width:10%;"><img class="tabelprezentare" src="imagini/muzee/<?php echo $row['poza'];?>"></td>
					<td><?php $sql2="SELECT * FROM `recenziimuzee` WHERE idInstitutie='".$row['idMuzeu']."';";  //
